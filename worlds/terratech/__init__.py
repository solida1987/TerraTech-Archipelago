...
from .Locations import (ALL_LOCATIONS, TerraTechLocation, attach_locations,
                        ...
                        pickup_locations, quest_locations,
                        shop_locations)
...
def _launch_client(*args: str) -> None:
    """Start the client, passing on anything the launcher was given.

    Arguments matter here: "--connect localhost:38281" turns four clicks into
    none, which is what lets London start a whole session for the player.
    """
    from .Client import launch
    launch_subprocess(launch, name="TerraTechClient", args=args)
...
class TerraTechWorld(World):
    ...
    def _fit_pool_to_locations(self) -> None:
        ...
        from .Locations import grade_locations as _gl
        _pool_corps = {b.corp for b in self.pool_blocks}
        grades_needed = sum(1 for name in _gl()
                            if name.split(" reaches Grade ")[0] in _pool_corps)
    ...
    def _location_budget(self) -> int:
        ...
        from .Data import QUEST_MILESTONES
        n = 0
        if self.options.pickup_checks:
            n += len(self.pool_blocks)
        if self.options.attach_checks:
            n += len(self.pool_blocks)
        # Grade locations exist only where the cap is measured (see
        # Data.GRADE_LOCATION_RANGE); counting five per corporation counted
        # locations that create_regions never makes.
        pool_corps = {b.corp for b in self.pool_blocks}
        n += sum(1 for name in grade_locations()
                 if name.split(" reaches Grade ")[0] in pool_corps)
    ...
    def create_items(self) -> None:
        ...
        pool_corps = {b.corp for b in self.pool_blocks}
        for corp, (lo, hi) in GRADE_LOCATION_RANGE.items():
    ...
    def fill_slot_data(self) -> dict:
        ...
        return {
            ...
            # corp=cap pairs for the corporations this seed tracks grades on.
            # A flat string on purpose: the mod's JSON reader is a scanner,
            # and one string survives it without a parser upgrade.
            "max_grades": ";".join(
                f"{corp}={hi}"
                for corp, (_, hi) in __import__(
                    "worlds.terratech.Data", fromlist=["GRADE_LOCATION_RANGE"]
                ).GRADE_LOCATION_RANGE.items()
                if corp in {b.corp for b in self.pool_blocks}),
