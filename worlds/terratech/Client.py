...
async def main(argv: tuple[str, ...]) -> None:
    parser = get_base_parser(description="TerraTech Archipelago client")
    # A slot name on the command line is what turns "start a session" into one
    # action. Added defensively: Archipelago may grow its own --name, and a
    # duplicate argument would kill the client on start-up.
    try:
        parser.add_argument("--name", default=None,
                            help="Slot name to connect as")
    except Exception:
        pass
    # parse_known_args, not parse_args: the Archipelago launcher passes the
    # component's own name through to the subprocess, and argparse exits(2) on
    # an argument it was not told about. The client then dies having written
    # exactly one line to its log, which looks like nothing happened at all.
    args, _unknown = parser.parse_known_args(list(argv))

    logger.info("Starting the TerraTech client.")
    ctx = TerraTechContext(args.connect, args.password)
    if getattr(args, "name", None):
        ctx.username = args.name
    ctx.server_task = asyncio.create_task(server_loop(ctx), name="ServerLoop")
    if gui_enabled:
        logger.info("Opening the client window.")
        ctx.run_gui()
    ctx.run_cli()
...
def launch(*args: str) -> None:
    """Start the client, and make sure a failure says so.

    Everything after logging is set up lives inside the guard, colorama
    included. A crash in the console-colour shim used to kill the process
    before a single useful line reached the log, which reads to a player as
    "clicking the client does nothing" -- the least actionable bug report
    there is.
    """
    Utils.init_logging("TerraTechClient", exception_logger="Client")
    logger.info("TerraTech client starting up.")
    try:
        try:
            import colorama
            colorama.just_fix_windows_console()
        except Exception:
            # Colour in a console we may not even have is not worth dying for.
            logger.debug("Console colour setup skipped.", exc_info=True)
            colorama = None
        try:
        ...
        logger.error("The TerraTech client exited early (code %r).", e.code)
        raise
    except BaseException:
        logger.exception("The TerraTech client stopped unexpectedly.")
        raise
